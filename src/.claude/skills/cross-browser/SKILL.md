---
name: cross-browser
description: Use when verifying page rendering or behavior in a non-Chromium engine (Safari/WebKit on desktop or iPhone, or Firefox), or comparing engines side by side; agent-browser covers Chromium.
---
...
- WebKit is the Safari engine but not Safari itself, and `--device` only emulates a phone, so platform chrome (form controls, scrollbars, font rendering) still differs; flag findings as WebKit-level, not Safari-confirmed
